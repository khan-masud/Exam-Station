...
import { 
  ...
  Zap, Flame, Users, BarChart3, Brain
} from "lucide-react"
...
export default function LeaderboardPage() {
  ...
  const [globalLeaderboard, setGlobalLeaderboard] = useState<GlobalRanking[]>([])
  ...
  useEffect(() => {
    if (selectedProgram && activeTab === "programs") {
      fetchProgramLeaderboard(selectedProgram)
    }
  ...
  const fetchGlobalLeaderboard = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/student/global-leaderboard', {
        credentials: 'include'
      })
      if (response.ok) {
        const data = await response.json()
  ...
  const fetchProgramLeaderboard = async (programId: string) => {
    try {
  ...
  const handleRefresh = async () => {
    ...
    if (activeTab === "global") {
      ...
      await fetchProgramLeaderboard(selectedProgram)
    } else if (activeTab === "overview") {
      await fetchMyPerformance(true)
    }
    setRefreshing(false)
  ...
  const getRankColor = (rank: number) => {
    ...
    if (rank === 3) return "bg-linear-to-r from-amber-400 to-amber-600"
    if (rank <= 10) return "bg-linear-to-r from-blue-400 to-blue-600"
    return "bg-muted"
  }
    ...
    <div className="p-6 pt-20 lg:pt-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Global rankings, program rankings, and your performance</p>
        </div>
        <Button 
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-3 max-w-2xl">
          ...
          <TabsTrigger value="programs">Program Rankings</TabsTrigger>
        ...
        <TabsContent value="global" className="space-y-4">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {entry.totalExams} exams completed
                      </p>
                    </div>
          ...
          {/* Global Insights */}
          {globalLeaderboard.length > 0 && globalLeaderboard.find(e => e.isCurrentUser) && (
            <Card className="border-l-4 border-l-blue-500">
              ...
              <CardContent className="space-y-3">
                <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded">
                  <p className="text-sm">
                    📊 You are ranked #{globalLeaderboard.find(e => e.isCurrentUser)?.rank} globally among all students
                  </p>
                </div>
                {globalLeaderboard.find(e => e.isCurrentUser)!.rank <= 10 && (
                  <div className="p-3 bg-green-50 dark:bg-green-950 rounded">
                    <p className="text-sm">
                      🌟 Excellent! You're in the top 10 globally!
                    </p>
                  </div>
                )}
                {globalLeaderboard.find(e => e.isCurrentUser)!.rank > 10 && (
                  <div className="p-3 bg-amber-50 dark:bg-amber-950 rounded">
                    <p className="text-sm">
                      💪 Keep going! Complete more exams to climb the global rankings
                    </p>
                  </div>
                )}
              </CardContent>
        ...
        <TabsContent value="programs" className="space-y-4">
          {programs.length === 0 ? (
              ...
              <CardContent className="pt-6">
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Enroll in programs to track your rankings
                  </p>
            ...
            <>
              ...
              {/* Program Leaderboard Insights */}
              {programLeaderboard.length > 0 && programLeaderboard.find(e => e.isCurrentUser) && (
                <Card className="border-l-4 border-l-purple-500">
                  ...
                  <CardContent className="space-y-3">
                    <div className="p-3 bg-purple-50 dark:bg-purple-950 rounded">
                      <p className="text-sm">
                        📊 You are ranked #{programLeaderboard.find(e => e.isCurrentUser)?.rank} out of {programLeaderboard.length} participants
                      </p>
                    </div>
                    {programLeaderboard.find(e => e.isCurrentUser)!.rank <= 10 && (
                      <div className="p-3 bg-green-50 dark:bg-green-950 rounded">
                        <p className="text-sm">
                          🌟 Great job! You're in the top 10 performers in this program
                        </p>
                      </div>
                    )}
                    {programLeaderboard.find(e => e.isCurrentUser)!.rank > 10 && programLeaderboard.length >= 10 && (
                      <div className="p-3 bg-amber-50 dark:bg-amber-950 rounded">
                        <p className="text-sm">
                          💪 Keep practicing to climb the leaderboard! You need{' '}
                          {Math.ceil(programLeaderboard[9]?.totalScore - programLeaderboard.find(e => e.isCurrentUser)!.totalScore)}{' '}
            ...
            <Card className="border-l-4 border-l-yellow-500">
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  #{myPerformance.length > 0 ? Math.min(...myPerformance.map(p => p.rank)) : '-'}
                </div>
            ...
            <CardContent className="space-y-4">
              {myPerformance.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mt-2">Enroll in programs and complete exams to see your rankings</p>
                </div>
              ) : (
